<?php
class AdminContactsListPage extends AbstractPageModule {
	...
	function doContent(){
		...
		if ($action == "delete" && !empty($id))	{
			...
			$query = $conn->newStatement("DELETE FROM contacts WHERE id=:id:");
	        $query->setInteger('id', $id);
	        $query->execute();
			...
			$this->response->redirect("/admin/contacts/list/{$page}/".($get_param?$get_param:""));
		}
		else {
			...
			if($this->request->getValue('count_page')){
				...
				define('COUNT_PAGE', 999);
			}
			require_once 'module/common/PagerFactory.class.php';
						
			$pager = new PagerFactory();
			$sql = "SELECT * FROM contacts ORDER BY pos DESC, id DESC";
			$fromWhereCnt = "contacts";
			$href = "/admin/contacts/list/".$get_param;
			...
			$this->response->write($this->renderTemplate('admin/admin_contacts_list.tpl'));
	...
	function ChangeCountPage($val, $get_param){
		...
		$objResponse->redirect("/admin/contacts/list/{$new_get_param}");
	...
	function Activate($id){
		...
		$query = $conn->newStatement("UPDATE contacts SET active=:active: WHERE id=:id:");
	...
	function Sort($mass_sort, $min_pos=1){ //  $min_pos - минимальное значение позиции на странице.
		...
		foreach ($mass_sort as $key => $value) {
			$query = $conn->newStatement("UPDATE contacts SET pos=:pos: WHERE id=:id:");
