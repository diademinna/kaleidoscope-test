...
class AdminNewsPhotoAddPage extends AbstractPageModule {
	...
	public function doContent(){
		...
		$this->template->assign("id_news", $id);
		
		$flag_swfupload = $this->request->getValue("swfupload");
		
		if (!empty($id) AND !$flag_swfupload){
			$conn = &DbFactory::getConnection();
			$query = $conn->newStatement("SELECT * FROM news_photo WHERE id_news=:id: ORDER BY pos DESC, id DESC");
		...
		if($flag_swfupload){ // значит заливаем картинки мультизагрузкой
				...
				if (!isset($_FILES["Filedata"]) || !is_uploaded_file($_FILES["Filedata"]["tmp_name"]) || $_FILES["Filedata"]["error"] != 0) {
					...
					FileSystem::createFolder("uploaded/news/{$id}", 0777);
					//if ($image['tmp_name']){ // Если картинку ЕСТЬ
						...
						$query_pos = $conn->newStatement("SELECT MAX(pos)+1 FROM news_photo");
						$pos = (int)$query_pos->getOneValue();
						if(!$pos){
							$pos = 1;
						}
						
						$query = $conn->newStatement('INSERT INTO news_photo SET id_news=:id_news:, ext=:ext:, name=:name:, pos=:pos:');
						...
						////
						ImageUtil::uploadImage($image['tmp_name'], "uploaded/news/{$id}/",  "{$id_new}_tmp.{$image_type}");
						$tmp_img = new ImageUtil("uploaded/news/{$id}/{$id_new}_tmp.{$image_type}");
						...
						if($image_width > 1024 AND ($img_position==1 OR $img_position==3) ){ // широкая
							$tmp_img->resizeProportionally("uploaded/news/{$id}/{$id_new}.{$image_type}", 1024, 1);
						}
						elseif ($image_height > 800 AND $img_position==2) { // высокая
							$tmp_img->resizeProportionallyHeight("uploaded/news/{$id}/{$id_new}.{$image_type}", 800, 1);
						}
						else{
							copy("uploaded/news/{$id}/{$id_new}_tmp.{$image_type}", "uploaded/news/{$id}/{$id_new}.{$image_type}");
						}
						...
						if($type_resize == 1){
							...
							if($img_position==1 OR $img_position==3){  // гориз.
								$tmp_img->ResizeFromRaf(200, 120, "uploaded/news/{$id}/{$id_new}_sm.{$image_type}");
							}
							else{ // вертик.
								$tmp_img->ResizeFromRaf(120, 200, "uploaded/news/{$id}/{$id_new}_sm.{$image_type}");
							}

							// РЕCАЙЗИТЬ ПРОПОРЦИОНАЛЬНО ШИРИНЕ!
							//$tmp_img->resizeProportionally("uploaded/news/{$id}/{$id_new}_sm.{$image_type}", 200, 1);
						}
						elseif ($type_resize == 2){
							//  РЕСАЙЗИТЬ по ОПРЕДЕЛЕННЫМ РАЗМЕРАМ С ПУСТЫМИ ПОЛЯМИ
							if($img_position==1 OR $img_position==3){  // гориз.
								$tmp_img->Resize(200, 120, "uploaded/news/{$id}/{$id_new}_sm.{$image_type}");
							}
							else{ // вертик.
								$tmp_img->Resize(120, 200, "uploaded/news/{$id}/{$id_new}_sm.{$image_type}");
							}
						}			

						FileSystem::deleteFile("uploaded/news/{$id}/{$id_new}_tmp.{$image_type}");

						$query = $conn->newStatement('UPDATE news_photo SET img_position=:img_position: WHERE id=:id:');
		...
		$this->response->write($this->renderTemplate("admin/admin_news_photo_add.tpl"));
	}
	...
	function deleteImage($id_news, $id_image){
		$xajax = new xajaxResponse();
		
		$conn = &DbFactory::getConnection();
		$query = $conn->newStatement("SELECT * FROM news_photo WHERE id={$id_image}");
		$data = $query->getFirstRecord();
		
		FileSystem::deleteFile("uploaded/news/{$id_news}/{$id_image}_sm.{$data['ext']}");
		FileSystem::deleteFile("uploaded/news/{$id_news}/{$id_image}.{$data['ext']}");
		
		$query = $conn->newStatement("DELETE FROM news_photo WHERE id={$id_image}");
		$query->execute();				
	...
	function reSaveName($formData, $id){
		...
		$conn =& DbFactory::getConnection();
		$query = $conn->newStatement("UPDATE news_photo SET name=:name: WHERE id=:id:");
	...
	function Sort($mass_sort, $min_pos=1){ //  $min_pos - минимальное значение позиции на странице.
		...
		foreach ($mass_sort as $key => $value) {
			$query = $conn->newStatement("UPDATE news_photo SET pos=:pos: WHERE id=:id:");
