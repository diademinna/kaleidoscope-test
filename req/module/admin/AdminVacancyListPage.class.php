<?php
class AdminVacancyListPage extends AbstractPageModule {
	...
	function doContent(){
		...
		if ($action == "delete" && !empty($id))	{			
			$query = $conn->newStatement("DELETE FROM vacancy WHERE id=:id:");
	        $query->setInteger('id', $id);
	        $query->execute();
	        
			$this->response->redirect("/admin/vacancy/list/{$page}/".($get_param?$get_param:""));
		}
		else {
			...
			$sql = "SELECT * FROM vacancy ORDER BY pos DESC, id DESC";
			$fromWhereCnt = "vacancy";
			$href = "/admin/vacancy/list/".$get_param;
			...
			$this->response->write($this->renderTemplate('admin/admin_vacancy_list.tpl'));
		}
	}
	...
	function ChangeCountPage($val, $get_param){
		...
		$objResponse->redirect("/admin/vacancy/list/{$new_get_param}");
	...
	function Activate($id){
		...
		$query = $conn->newStatement("UPDATE vacancy SET active=:active: WHERE id=:id:");
	...
	function Sort($mass_sort, $min_pos=1){ //  $min_pos - минимальное значение позиции на странице.
		...
		foreach ($mass_sort as $key => $value) {
			$query = $conn->newStatement("UPDATE vacancy SET pos=:pos: WHERE id=:id:");
