...
{capture name="content"}
	...
	<script type="text/javascript">
		function delRecord(page, id, get_param){
			if(confirm("Вы уверены?")) {
				top.window.location = "/admin/article/list/"+page+"/delete/"+id+"/"+get_param;
			}
		}
	...
	<table class="list" width="100%">
		...
		<tr class="nobgr">
			<td></td>
			<td style="text-align:right;"><a class="add" href="/admin/article/add/{if $get_param}{$get_param}{/if}">Добавить</a></td>
		</tr>
					...
					<ul id="sortable"  class="sort">
						{foreach from=$data item=cur name=loop}
							<li id="item_{$cur.id}">								
								...
								<div style="width:15%;"><div class="padd">									
									<a href="/admin/article_photo/add/{$page}/{$cur.id}/{if $get_param}{$get_param}{/if}"><img src="/img/admin/photo.png"  title="Галерея" alt="Галерея" /></a> 
									
									<a href="/admin/article/add/{$page}/edit/{$cur.id}/{if $get_param}{$get_param}{/if}"><img src="/img/admin/edit.png"  title="Редактировать" alt="Редактировать" /></a>
