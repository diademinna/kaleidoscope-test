...
{capture name="content"}
	{literal}
		<script type="text/javascript">
			function del(id_article, id_image)	{
				if(confirm("Вы уверены?")) {
					xajax_deleteImage(id_article, id_image);
				}
			}
	...
	<form action="" method="post" enctype="multipart/form-data">
		...
		<table class="edit" width="100%">
			<tr>
				<td><a href="/admin/article/list/{$page}/{if $get_param}{$get_param}{/if}" class="back"><span></span>НАЗАД</a><br><br></td>
				<td>
	...
	<script type="text/javascript">
		...
		function uploadStart() {	// функция вызывается перед загрузкой фоток				
			this.setPostParams({"id_from_swfupload": $("#id_article").val(),								
		...
		window.onload = function() {
			...
			var settings = {
				...
				upload_url: "/admin/article_photo/add/{/literal}{$page}{literal}/{/literal}{$id_article}{literal}/?swfupload=1",
				//post_params: { "id_from_swfupload": '{/literal}{$id_article}{literal}'},
		...
		<div class="sort_list gall_block">
			...
			<ul id="sortable"  class="sort">
				{foreach from=$data_photo item=cur name=loop}
					...
					<li id="item_{$cur.id}">
						<form id="form_item_{$id_article}" name="form_img_{$id_article}" action="" method="post">
							<div style="width:55%;"><div class="padd">
									<a href="/uploaded/article/{$cur.id_article}/{$cur.id}.{$cur.ext}" target="_blank"><img src="/uploaded/article/{$cur.id_article}/{$cur.id}_sm.{$cur.ext}" alt="{$cur.name}" title="{$cur.name}" class="photo" /></a>
								<img class="pointer" src="/img/admin/del.png" onclick="del('{$cur.id_article}','{$cur.id}');" title="Удалить изображение" alt="Удалить изображение" style="cursor:pointer;" /><br><br>
							</div></div>

							<div style="width:45%;"><div class="padd">									
