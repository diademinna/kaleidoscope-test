...
{capture name="content"}
	{literal}
		<script type="text/javascript">
			function del(id_blog, id_image){
				if(confirm("Вы уверены?")) {
					xajax_deleteImage(id_blog, id_image);
				}
			}
	...
	<form action="" method="post" enctype="multipart/form-data">
		...
		<table class="edit" width="100%">
			<tr>
				<td><a href="/admin/blog/list/{$parent_id}/page/{$page}/{if $get_param}{$get_param}{/if}" class="back"><span></span>НАЗАД</a><br><br></td>
				<td>
			...
			<tr>
				<td></td>
				<td>
					<input type="hidden" name="id_blog" id="id_blog" value="{$id_blog}" />
	...
	<script type="text/javascript">
		...
		function uploadStart() {	// функция вызывается перед загрузкой фоток				
			this.setPostParams({"id_from_swfupload": $("#id_blog").val(),								
		...
		window.onload = function() {
			...
			var settings = {
				...
				upload_url: "/admin/blog_photo/add/{/literal}{$parent_id}{literal}/edit/{/literal}{$id_blog}{literal}/page/{/literal}{$page}{literal}/?swfupload=1",
		...
		<div class="sort_list gall_block">
			...
			<ul id="sortable"  class="sort">
				{foreach from=$data_photo item=cur name=loop}
					...
					<li id="item_{$cur.id}">
						<form id="form_item_{$id_blog}" name="form_img_{$id_blog}" action="" method="post">
							<div style="width:50%;"><div class="padd">
									<a href="/uploaded/blog/{$cur.id_blog}/{$cur.id}.{$cur.ext}" target="_blank"><img src="/uploaded/blog/{$cur.id_blog}/{$cur.id}_sm.{$cur.ext}" alt="{$cur.name}" title="{$cur.name}" class="photo" /></a>
								<img class="pointer" src="/img/admin/del.png" onclick="del('{$cur.id_blog}','{$cur.id}');" title="Удалить изображение" alt="Удалить изображение" style="cursor:pointer;" /><br><br>
							</div></div>
