{capture name="content_name"}
	Каталог / {if $data.name}Редактировать - {$data.name}{else} Добавить
        {if $level}
            парметр  
        {else}
            группу параметров
	...
	<form action="" method="post" class="form-horizontal" enctype="multipart/form-data">
		...
		<div class="form-group">
                    ...
                    <div class="col-sm-10">
                        ...
                        <div id="photo">
                            {if $data.ext}
                                <a href="/uploaded/product_param/{$data.id}_sm.{$data.ext}" target="_blank"><img src="/uploaded/product_param/{$data.id}_sm.{$data.ext}" class="photo" /></a>
