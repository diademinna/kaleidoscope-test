	...
	<script type="text/javascript">
		...
		window.onload = function() {
			...
			var settings = {
				...
				upload_url: "/admin/product_photo/add/{/literal}{$id_product}{literal}/?swfupload=1",
				//post_params: { "id_from_swfupload": '{/literal}{$id_product}{literal}'},
	...
	{if $data_photo}	
		...
		<div class="sort_list gall">
				...
				{foreach from=$data_photo item=cur name=loop}
					...
					<li id="item_{$cur.id}">
						<form id="form_item_{$id_product}" name="form_img_{$id_product}" action="" method="post">
                                                    <div class='row'>
							<div class='col-sm-4'>
                                                            <div class="padd">
									<a href="/uploaded/product/{$cur.id_product}/{$cur.id}.{$cur.ext}" target="_blank"><img src="/uploaded/product/{$cur.id_product}/{$cur.id}_sm.{$cur.ext}" alt="{$cur.name}" title="{$cur.name}" class="photo" /></a>
								<img class="pointer" src="/img/admin/del.png" onclick="del('{$cur.id_product}','{$cur.id}');" title="Удалить изображение" alt="Удалить изображение" style="cursor:pointer;" /><br><br>
							</div>
                                                        </div>
